-- ----------------------------------------------------------------
-- gpsSeasonDrivers
-- This table stores one entry per driver per season
-- ----------------------------------------------------------------
...
select d.driverId, d.driverRef, s.year, d.forename, d.surname,
       ...
       (select position from gpsDriverStandings ds where ds.driverId = d.driverId and year = s.year and round = s.rounds) as standingsPosition,
       (select count(1) from gpsDriverStandings ds where ds.driverId = d.driverId and position = 1 and year = s.year and round = s.rounds) as championships,
       (select count(1) from gpsDriverStandings ds where ds.driverId = d.driverId and position = 2 and year = s.year and round = s.rounds) as subChampionships,
